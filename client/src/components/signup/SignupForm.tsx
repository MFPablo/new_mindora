...
export function SignupForm() {
  ...
  const [role, setRole] = useState<"patient" | "professional">(plan ? "professional" : "patient");
  ...
  const {
    ...
    defaultValues: {
      role: plan ? "professional" : "patient",
    },
  });

  // Sync state role to form role
  useEffect(() => {
    setValue("role", role);
  }, [role, setValue]);
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 gap-4">
          <button
            ...
            className={`rounded-xl border-2 p-4 text-center transition-all ${role === "patient"
              ? "border-blue-600 bg-blue-50 dark:bg-blue-900/20"
              : "border-gray-200 dark:border-gray-700 hover:border-blue-600/50"
              }`}
          >
            <div className={`mx-auto h-10 w-10 rounded-full flex items-center justify-center mb-2 transition-colors ${role === "patient" ? "bg-blue-600 text-white" : "bg-gray-100 dark:bg-gray-800 text-gray-500"
              }`}>
          ...
          <button
            ...
            className={`rounded-xl border-2 p-4 text-center transition-all ${role === "professional"
              ? "border-blue-600 bg-blue-50 dark:bg-blue-900/20"
              : "border-gray-200 dark:border-gray-700 hover:border-blue-600/50"
              }`}
          >
            <div className={`mx-auto h-10 w-10 rounded-full flex items-center justify-center mb-2 transition-colors ${role === "professional" ? "bg-blue-600 text-white" : "bg-gray-100 dark:bg-gray-800 text-gray-500"
              }`}>
        ...
        <div className="flex items-start">
          ...
          <div className="ml-3 text-sm">
            <label className="font-medium text-gray-700 dark:text-gray-300" htmlFor="terms">
              Acepto los <Link to="/" className="text-brand-primary hover:text-blue-700">Términos de Servicio</Link> y la <Link to="/" className="text-brand-primary hover:text-blue-700">Política de Privacidad</Link>
            </label>
          </div>
        </div>
